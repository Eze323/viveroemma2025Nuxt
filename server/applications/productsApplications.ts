...
//buscaTodos
export const buscaTodos = async (event: H3Event):Promise<products[] | string> => {
...
export const buscarPorId = async (event : H3Event): Promise<products|string> =>{
...
//Crea
export const crear = async (event: H3Event): Promise<products | string> => {
    const body = await readBody<products>(event); //leo el body y lo tipifico como products
