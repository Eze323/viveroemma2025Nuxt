export const useApiService = () => {
  ...
  const normalizeResponse = (response) => {
    ...
    if (Array.isArray(response)) {
      return response
        .map((item) => {
          ...
          return item; // Devolvemos el item sin normalizar para evitar perder datos
        })
    ...
    if (response.sale) {
      return normalizeSale(response.sale);
    }
    ...
    if ('customer' in response || ('customer_name_field' in response && 'total_price' in response)) {
      return normalizeSale(response);
    }

    if ('name' in response && 'category' in response) {
      return normalizeProduct(response);
    }

    if ('invoice_number' in response && 'total_amount' in response) {
      return normalizeInvoice(response);
    }

    if ('name' in response && !('category' in response) && 'id' in response) {
